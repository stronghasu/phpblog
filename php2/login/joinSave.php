    ...
    <main id="contents">
        ...
        <?php
            ...
            if($check_number == false){
                msg("번호가 정확하지 않습니다. <br>올바른 생년월일(000-0000-0000) 적어주세요!");
            }
            ...
            $sql = "SELECT youEmail FROM harryMember WHERE youEmail = '$youEmail'";
            $result = $connect -> query($sql);
            ...
            if($result) {
                ...
                */
                ...
                // num_rows : 해당요소의 각 열에 반환할 데이터가 존재하는지 확인 
            ...
            $sql = "SELECT youPhone FROM harryMember WHERE youPhone = '$youPhone'";
            $result = $connect -> query($sql);
            ...
            //성별 
            $sql = "SELECT youGender FROM harryMember WHERE youGender = '$youGender'";
            $result = $connect -> query($sql);
          
            //닉네임 중복검사 
            $isNickCheck = false;
            $check_nick = preg_match("/^[\w\Wㄱ-ㅎㅏ-ㅣ가-힣]{2,20}$/", $youNickName);
            
            $sql = "SELECT youNickName FROM harryMember WHERE youNickName = '$youNickName'";
            $result = $connect -> query($sql);
            ...
            //자기소개 
            $sql = "SELECT youIntro FROM harryMember WHERE youIntro = '$youIntro'";
            $result = $connect -> query($sql);

            //회원가입
            if($isEmailCheck == true && $isPhoneCheck == true){
                $sql = "INSERT INTO harryMember(youEmail, youPass, youNickName, youName ,youBirth, youPhone, youGender, youIntro, regTime) VALUES('$youEmail', '$youPass', '$youNickName', '$youName', '$youBirth', '$youPhone', '$youGender', '$youIntro', '$regTime')";
                $result = $connect -> query($sql);
